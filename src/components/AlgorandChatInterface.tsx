...
import { Send, Mic, MicOff, Volume2, Bot, Lightbulb, TrendingUp, BarChart3 } from 'lucide-react';
import { Character } from '../types';
import { useGeminiChat } from '../hooks/useGeminiChat';
import { Button } from '../ui';

interface AlgorandChatInterfaceProps {
  character: Character;
}

const AlgorandChatInterface: React.FC<AlgorandChatInterfaceProps> = ({ character }) => {
  const [message, setMessage] = useState('');
  const { messages, isTyping, isRecording, sendMessage, startVoiceRecording, stopVoiceRecording, generateInsight } = useGeminiChat(character.id);
  ...
  const quickQuestions = [
    "What's the current state of Algorand DeFi?",
    "How does Algorand's consensus mechanism work?",
    "What are the best ALGO trading strategies?",
    "Explain Algorand Standard Assets (ASAs)",
    "What's new in the Algorand ecosystem?"
  ];
    ...
    <div className="flex flex-col h-full bg-gradient-to-br from-gray-900 to-gray-800">
      ...
      <div className="flex items-center space-x-4 p-6 border-b border-gray-700/50 bg-gray-800/50 backdrop-blur-md">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-400">Algorand Blockchain Strategist</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        ...
        {messages.length <= 1 && (
          <motion.div
            ...
            className="space-y-3"
          >
            <p className="text-sm text-gray-400 text-center">Quick questions to get started:</p>
            <div className="grid grid-cols-1 gap-2">
              {quickQuestions.map((question, index) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="p-3 text-left text-sm bg-gray-700/30 hover:bg-gray-700/50 border border-gray-600/30 rounded-lg text-gray-300 transition-all duration-200"
                >
                  {question}
                </motion.button>
              ))}
        ...
        <form onSubmit={handleSendMessage} className="flex items-center space-x-4">
          <div className="flex-1 relative">
            <input
              ...
              placeholder={`Ask ${character.name} about Algorand...`}
...
export default AlgorandChatInterface;